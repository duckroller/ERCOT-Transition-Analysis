---
title: "All the Clusters!"
output: html_notebook
---

#More fun with clustering!

Here we're trying to do a lot of clusters, and figure out which are closest to the EPA's regional groups of states.
...
source("multivariate_hcl.R")
```

```{r}
# Pulling in EPA clusters. These 9 regions are used in our model, and are strictly based on geographic location.
rm(epa_clusters)
epa_clusters <- read_csv("state_to_code.csv", col_names = c("state","epa_region"),skip=1)
epa_clusters$state <- state.name[match(epa_clusters$state,state.abb)]
epa_clusters$epa_region <- substring(epa_clusters$epa_region,2,2)
epa_clusters <- data.frame(na.omit(epa_clusters))
```

```{r}
# Now, let's pull in our nice variables to cluster on.
...
emissions_data <- merge(emissions_data,epa_clusters,by="state")
...
allvars_km <- kmeans(emissions_data[,1:14], 3, nstart = 25)

ed <- merge(emissions_data[,1:14], allvars_km$cluster, by = 0)

rownames(ed) <- ed$Row.names
ed$Row.names <- NULL

ed$clus <- as.character(ed$y)
ed$y <- NULL

ggpairs(ed, mapping=ggplot2::aes(colour = clus))

```


```{r}
# Enumerating all the pairs of variables.
bivariate_pairs <- subsets(13,2,names(emissions_data)[1:14])
```


```{r}
#A <- lapply (1 : 78, function (x) bivariate_clustering(data.frame(emissions_data[,bivariate_pairs[x,]]),min_nodes = 2))

n <- nrow(bivariate_pairs)

A <- lapply (1 : n, function (x) kmeans(data.frame(emissions_data[,bivariate_pairs[x,]]), 9, nstart = 25))
```
...
dist_em_data <- dist(emissions_data[1:13], diag = TRUE)

#cluster.stats(dist_em_data,clustering=A[[1]]$cluster,alt.clustering = emissions_data$epa_region)

B <- lapply(1:length(A), function (x) cluster.stats(dist_em_data,clustering=A[[x]]$cluster,alt.clustering = emissions_data$epa_region))
```


```{r}
# Rand of 1: same clustering. Rand of 0: No agreement on clusters.
rands <- sapply(1:length(B), function (x) B[[x]]$corrected.rand)

max(rands)
which.max(rands)
bivariate_pairs[which.max(rands),]
```

```{r}
# Low VI: good. Higher VI: Bad.
vis <- sapply(1:length(B), function (x) B[[x]]$vi)

min(vis)
which.min(vis)
bivariate_pairs[which.min(vis),]
```


```{r}
fviz_cluster(A[[which.min(vis)]], data=emissions_data[,1:13])

# EPA clusters
fviz_cluster(object = list(data = emissions_data[,1:13], cluster = emissions_data[,14]))
```


```{r}

mass_clustering <- function (ed = emissions_data, varible_ct = 2){
  

  # Enumerating all the pairs of variables.
  variable_selection <- subsets(13,varible_ct,names(ed)[1:13])
  
  n <- nrow(variable_selection)
  
  A <- lapply (1 : n, function (x) kmeans(data.frame(ed[,variable_selection[x,]]), 9, nstart = 25))
  
  library(fpc)
  
  dist_em_data <- dist(ed[1:13], diag = TRUE)
  
  #cluster.stats(dist_em_data,clustering=A[[1]]$cluster,alt.clustering = emissions_data$epa_region)
  
  B <- lapply(1:length(A), function (x) cluster.stats(dist_em_data,clustering=A[[x]]$cluster,alt.clustering = ed$epa_region))
  
  # Rand of 1: same clustering. Rand of 0: No agreement on clusters.
  rands <- sapply(1:length(B), function (x) B[[x]]$corrected.rand)
  
  best_rands_val <- max(rands)
  #which.max(rands)
  best_rands_variables <- variable_selection[which.max(rands),]
  ...
  best_VI_val <- min(vis)
  #which.min(vis)
  best_vi_variables <- variable_selection[which.min(vis),]
  
  
  closest_cluster_graph <- fviz_cluster(A[[which.min(vis)]], data=ed[,1:13])
  
  #fviz_cluster(object = list(data = ed[,1:13], cluster = ed[,14]))
  
  return(list(variables = varible_ct, rand_val = best_rands_val, rand_vars = best_rands_variables, vi_val = best_VI_val, vi_vars = best_vi_variables, graph = closest_cluster_graph))
}
```

```{r}

C <- lapply(2:12, function (y) mass_clustering(varible_ct = y))
```

```{r}
rands <- sapply(1:length(C), function (x) C[[x]]$rand_val)
  
best_rands_val <- max(rands)

vis <- sapply(1:length(C), function (x) C[[x]]$vi_val)
  
best_vi_val <- min(vis)
```

```{r}
bivariate_pairs <- subsets(13,2,names(emissions_data)[1:13])

D <- lapply(2:12, function (y) multivariate_hcl(emissions_data, var_count = y))
```

```{r}
closest_to_epa <- kmeans(emissions_data[,c(7,9:11)], centers = 9, nstart = 25)


fviz_cluster(closest_to_epa, data = emissions_data[,c(7,9:11)], main = "State Clusters Closest to EPA Regions")
```

```{r}
k9$state <- rownames(k9)
 ussc <- full_join(us_states, k9, by = c("region" = "state"))
 

 c_map <- ggplot() +
                 geom_polygon(data=k9,
                              aes(x=long, 
                                  y=lat, 
                                  group = group, 
                              ...
                              color="grey50") +
                 labs(fill = "Cluster")+
                 ggtitle("Geographical Representation")+
...
```
```{r}
badNbClust <- function (data = NULL, diss = NULL, distance = "euclidean", 
...
```{r}
E <- lapply(1:n, function (x) badNbClust(data = data.frame(emissions_data[,bivariate_pairs[x,]]), diss = NULL, distance = "euclidean", min.nc = 2, max.nc = 10, method = "complete"))
```
